...
void print_process(string line);

char *format_state(string state);
...
main()
{
	...
	if (!procs) {
		perror("Error: No se pude abrir el directorio /proc");
		exit(-1);
	}
	struct dirent *entry;
	while ((entry = readdir(procs))) {
		// Los procesos seran accesibles en forma de directorio dentro
		// de /procs Los directoris de los procesos se llaman por su
		// PID, entonces filtramos los directorios con nombre numerico
...
format_state(string state)
{
	...
	if (!strcmp(state, "Z"))
		return "Zombie";
	if (!strcmp(state, "T"))
		return "Stopped (on a signal) or (before Linux 2.6.33) trace "
		       "stopped";
	if (!strcmp(state, "t"))
