...
read_line(const char *promt)
{
	...
	struct winsize w;
	ioctl(STDOUT_FILENO, TIOCGWINSZ, &w);  // Load winsize obj
	int MAX_COL =
	        w.ws_col -
	        3;  // Get width - 3 char of leeway ( locally tested to be ok )
...
void
handleEsc(int *i, size_t *row, size_t *col, int MAX_COL)
{
	int c = 0;
	read(STDIN_FILENO, &c, 1);
	switch (c) {
...
void
handleBackspace(int *i, size_t *row, size_t *col, int MAX_COL)
{
	if (*i <= 0)
		return;
...
void
writeWord(char *str, size_t *row, size_t *col, int MAX_COL)
{
